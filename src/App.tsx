...
export default function App() {
  ...
  const [localModels, setLocalModels] = useState<{model:string;label:string;available:boolean}[]>([]);
  const [mcpAgentPrompt, setMcpAgentPrompt] = useState("Query active database tables and alert slack of table changes");

  useEffect(() => {
    fetch("/api/agent/local-models")
      .then(r => r.json())
      .then(d => { if (d.models?.length > 0) setLocalModels(d.models); })
      .catch(() => {});
  }, []);
  ...
  // the Performance API (see the observer effect below) Ã¢ÂÂ not fabricated and
  ...
  // Real Core Web Vitals Ã¢ÂÂ measured live in this browser via the Performance
            ...
            {projectsList.length > 0 && currentProject && (
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <div className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-neutral-500 text-xs">Ã¢ÂÂ¼</div>
                </div>
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div className="p-3 bg-neutral-950 rounded-lg text-[10px] text-neutral-400 space-y-1">
                        {shieldConfig.sslMode === "off" && (
                          <span>Ã¢ÂÂ Ã¯Â¸Â Danger: Visitor requests bypass SSL standard limits. Passwords & cookies travel as cleartext.</span>
                        )}
                        {shieldConfig.sslMode === "flexible" && (
                          <span>Ã¢ÂÂ¡ Standard proxy mode. SSL completes in 140+ Anycast Vortex hubs. Origin may answer over plain HTTP.</span>
                        )}
                        {shieldConfig.sslMode === "full" && (
                          <span>Ã°ÂÂÂ Direct end-to-end 2048-bit TLS handshake. Requests remain encrypted until they enter container memory.</span>
                        )}
                        {shieldConfig.sslMode === "strict" && (
                          <span>Ã°ÂÂÂ¡Ã¯Â¸Â Maximum corporate audit enforcement. Requires valid trust certificates mapped explicitly under settings.</span>
                        )}
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="p-3 bg-neutral-950 rounded-lg text-[10px] text-neutral-400 space-y-1">
                        {shieldConfig.securityLevel === "under-attack" ? (
                          <span className="text-rose-400 font-semibold flex items-center gap-1.5 animate-pulse">
                            Ã°ÂÂÂ¨ Mitigating DDoS attack. Browsers must complete automatic 3-second Javascript challenges.
                          </span>
                        ) : (
                          <span>Ã°ÂÂÂ¡Ã¯Â¸Â Regular protection mode active. Safe crawlers and legitimate visitors pass uninhibited.</span>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="px-5 py-4 border-b border-neutral-800 bg-neutral-950/40 flex justify-between items-center">
                    ...
                    {shieldConfig.securityLevel === "under-attack" && (
                      <span className="text-[10px] text-rose-400 font-bold flex items-center gap-1.5 animate-pulse">
                        Ã¢ÂÂ LIVE FIREWALL BLOCKS BUFFERING
                      </span>
                    )}
                  </div>
                            ...
                            <div className="p-3 bg-neutral-950/20 border-b border-neutral-850/85 flex flex-wrap gap-2 text-[10px] text-neutral-505 uppercase font-bold">
                              {selectedDbTable.columns.map((c: any) => (
                                <span key={c.name} className="px-2 py-0.5 bg-neutral-950 border border-neutral-850 rounded">
                                  Ã°ÂÂÂ {c.name}: <span className="text-neutral-400 font-semibold">{c.type}</span>
                                  {c.isPrimaryKey && <span className="text-amber-500 ml-1">Ã¢ÂÂ PK</span>}
                                </span>
                              ))}
                            </div>
                              ...
                              <div className="p-4 bg-neutral-950 border border-neutral-850 rounded-lg text-xs leading-relaxed space-y-2 select-all overflow-auto font-mono">
                                <div className="flex items-center justify-between border-b border-neutral-900 pb-1.5 mb-1 text-[10px] text-neutral-500 font-bold">
                                  ...
                                  <span className={dbQueryResult.success !== false ? "text-emerald-400" : "text-rose-455"}>
                                    {dbQueryResult.success !== false ? "Ã¢ÂÂ COMPLETE SUCCESS" : "Ã¢ÂÂ EXHAUSTED ERROR"}
                                  </span>
                                </div>
                        ...
                        <div className="space-y-3">
                          <div className="space-y-1">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {[
                                { id: "postgresql", label: "PostgreSQL", desc: "SQL relational", icon: "Ã°ÂÂÂ" },
                                { id: "mysql", label: "MySQL", desc: "Standard SQL", icon: "Ã°ÂÂÂ¬" },
                                { id: "mongodb", label: "MongoDB", desc: "NoSQL document", icon: "Ã°ÂÂÂ" },
                                { id: "redis", label: "Redis DB", desc: "Key-value cache", icon: "Ã¢ÂÂ¡" }
                              ].map((eng) => (
                                <button
                                  key={eng.id}
                                  type="button"
                          ...
                          <div className="space-y-1">
                            ...
                            >
                              ...
                              <option value="SA-East-1 (SÃÂ£o Paulo)">SA-East-1 (SÃÂ£o Paulo)</option>
                            </select>
                          </div>
                          ...
                          >
                            {isProvisioningDb ? (
                              ...
                              "Ã¢ÂÂ¡ PROVISION CLOUD INSTANCE"
                            )}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="xl:col-span-2 space-y-4 font-mono text-xs">
                        {databaseServices.length === 0 ? (
                          <div className="bg-neutral-900 border border-neutral-800 p-10 rounded-xl text-center space-y-3 shadow-sm font-mono text-xs">
                            <span className="text-3xl">Ã°ÂÂÂÃ¯Â¸Â</span>
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-800 pb-3 mb-4 font-mono">
                                  <div className="flex items-center gap-2">
                                    <span className="text-lg">
                                      {srv.type === "postgresql" ? "Ã°ÂÂÂ" : srv.type === "mysql" ? "Ã°ÂÂÂ¬" : srv.type === "mongodb" ? "Ã°ÂÂÂ" : "Ã¢ÂÂ¡"}
                                    </span>
                                    <div>
                      ...
                      <div className="lg:col-span-1 bg-neutral-900 border border-neutral-800 rounded-xl p-5 space-y-4 shadow-sm">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-1.5">
                            ...
                            >
                              <option value="256MB">256MB Micro Ã¢ÂÂ Free Tier</option>
                              <option value="512MB">512MB Medium Ã¢ÂÂ Standard</option>
                              <option value="1GB">1GB Large Ã¢ÂÂ Professional Scale</option>
                              <option value="2GB">2GB Extreme Ã¢ÂÂ Production swarm</option>
                            </select>
                          </div>
                      ...
                      <div key={usr.id} className="p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4 transition hover:bg-neutral-950/15">
                        <div className="space-y-1.5 min-w-0">
                          ...
                          <p className="text-[10px] text-neutral-500">
                            Registered: <span className="text-neutral-400">{new Date(usr.createdAt).toLocaleString()}</span> Ã¢ÂÂ¢ Last Session: <span className="text-neutral-400">{new Date(usr.lastLogin).toLocaleString()}</span>
                          </p>
                        </div>
                ...
                <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-8 space-y-6 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <span className="text-[10px] text-emerald-500/80 block leading-normal">
                        No PC or VPS required! Run programmatically in the cloud container. Zero authentication requiredÃ¢ÂÂdirectly paste into LobeChat SaaS, Coze, or Dify.
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      <label className="text-xs uppercase font-bold text-neutral-500 font-mono tracking-wider">
                        Ã°ÂÂÂ¡Ã¯Â¸Â ACTIVE SECURITY GUARDS
                      </label>
                  ...
                  <div className="mt-8 border-t border-neutral-800 pt-6 space-y-6">
                    ...
                    <div className="flex border-b border-neutral-800 gap-2 overflow-x-auto">
                      ...
                      >
                        Ã¢ÂÂÃ¯Â¸Â Cloud & Serverless SaaS
                      </button>
                                {mcpAgentGuideTab === "claude" && (
                        <div className="space-y-4">
      ...
      "args": [
        ...
        `${mcpPublicUrl || window.location.origin + "/api/mcp/sse"}`
                          ...
                          <div className="text-[11px] text-neutral-500 leading-normal">
                            Ã°ÂÂÂ¡ **Where is the configuration file?**
                      ...
                      {mcpAgentGuideTab === "cursor" && (
                        <div className="space-y-4 text-xs text-neutral-300 leading-relaxed">
                          ...
                          <p className="text-[11px] text-neutral-500 leading-normal">
                            Ã°ÂÂÂ Once added, Cursor's AI Composer and Chat will immediately gain native tool execution access to list and modify database tables, environments, container configurations, and domains.
                          </p>
                        </div>
                      )}
                        ...
                        <div className="space-y-5 text-xs text-neutral-300 leading-relaxed font-sans">
                          <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 space-y-2">
                            <h4 className="text-amber-400 font-bold flex items-center gap-2">
                              Ã¢ÂÂ Ã¯Â¸Â Understanding the 302 Authentication Wall
                            </h4>
                          ...
                          <div className="space-y-3">
                            <h4 className="text-emerald-400 font-bold uppercase tracking-wider text-[11px]">
                              Ã°ÂÂÂ 100% Serverless Solution (No PC or VPS Needed)
                            </h4>
                            <p>
                          ...
                          <div className="space-y-3">
                            <h4 className="text-emerald-400 font-bold uppercase tracking-wider text-[11px]">
                              Ã°ÂÂÂ Compatible Cloud & SaaS Clients
                            </h4>
                            <p>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2.5">
                                <div className="h-8 w-8 bg-neutral-900 border border-neutral-800 rounded-lg flex items-center justify-center text-neutral-100 font-sans select-none">
                                  {connector.logo === "slack" && "Ã°ÂÂÂ¬"}
                                  {connector.logo === "github" && "Ã°ÂÂÂ"}
                                  {connector.logo === "discord" && "Ã°ÂÂÂ¾"}
                                  {connector.logo === "notion" && "Ã°ÂÂÂ"}
                                  {connector.logo === "stripe" && "Ã°ÂÂÂ³"}
                                  {connector.logo === "hubspot" && "Ã°ÂÂÂ¯"}
                                  {connector.logo === "gmail" && "Ã¢ÂÂÃ¯Â¸Â"}
                                  {connector.logo === "salesforce" && "Ã¢ÂÂÃ¯Â¸Â"}
                                </div>
                                <div>
                            ...
                            {connector.isConnected && (
                              <div className="flex items-center justify-between border-t border-neutral-900 pt-2 text-[10px] text-neutral-500 font-mono">
                                ...
                                <span className="text-emerald-400 select-none">Ã¢ÂÂ READY</span>
                              </div>
                            )}
                          </div>
                        ))
                  ...
                  <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 space-y-6 shadow-sm">
                    ...
                    <div className="space-y-3 font-mono text-xs">
                      <div className="space-y-1.5">
                        ...
                        >
                    <optgroup label="── Gemini (needs GEMINI_API_KEY) ──">
                        ...
                        <optgroup label="── GLM via Z.ai (needs GLM_API_KEY) ──">
                          <option value="glm:glm-5.2">GLM-5.2 — Z.ai API · 1M ctx</option>
                        </optgroup>
                        <optgroup label="── Local / No API Key ──">
                          <option value="ollama:glm4">GLM-4:9B via Ollama — ollama pull glm4</option>
                          <option value="ollama:glm4.7-flash">GLM-4.7-Flash via Ollama — ollama pull glm4.7-flash</option>
                          <option value="llamaserver:glm-5.2">GLM-5.2 via llama-server — set GLM_UNCENSORED_URL</option>
                          {localModels.filter(m => m.available).map(m => (
                        ...
                        >
                          <div className="space-y-1 font-mono text-xs">
                            ...
                            <span className="text-neutral-300 font-semibold select-all">
                              {isRevealed ? env.value : "Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢Ã¢ÂÂ¢"}
                            </span>
                          </div>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[9px] text-emerald-400">Ã¢ÂÂ SECURED VAULT</span>
                          </div>
                          <input
                            type="password"
