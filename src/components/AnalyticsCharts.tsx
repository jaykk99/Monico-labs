...
export default function AnalyticsCharts({
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === 'update') {
        setUpdatedMetrics(data.metrics);
      }
    };
    ws.onerror = (event) => {
  ...
  const maxRequests = Math.max(...updatedMetrics.map((m) => m.requests), 1);
  const maxBandwidth = Math.max(...updatedMetrics.map((m) => m.bandwidth), 1);
  const maxLatency = Math.max(...updatedMetrics.map((m) => m.latency), 1);
  ...
  const reqPoints = getPoints(updatedMetrics.map((m) => m.requests), maxRequests);
  const bandPoints = getPoints(updatedMetrics.map((m) => m.bandwidth), maxBandwidth);
  const latPoints = getPoints(updatedMetrics.map((m) => m.latency), maxLatency);
  ...
  const activeMetric = hoverIndex !== null ? updatedMetrics[hoverIndex] : updatedMetrics[updatedMetrics.length - 1];
              ...
              <div className='p-4 bg-neutral-950/40 border border-neutral-800/45 rounded-xl space-y-2'>
                ...
                <div className='h-[75px] w-full bg-neutral-950/50 rounded-lg relative overflow-hidden'>
                  <svg className='w-full h-full overflow-hidden' viewBox='0 0 280 75' preserveAspectRatio='none'>
                    <path
                      d={getAreaPathString(
                        getPoints(updatedMetrics.map((m) => m.bandwidth), maxBandwidth).map((p) => ({ x: p.x / 2, y: p.y / 2 }))
                      )}
                      fill='#10b981'
                      fillOpacity='0.08'
                    />
              ...
              <div className='p-4 bg-neutral-950/40 border border-neutral-800/45 rounded-xl space-y-2'>
                ...
                <div className='h-[75px] w-full bg-neutral-950/50 rounded-lg relative overflow-hidden'>
                  <svg className='w-full h-full overflow-hidden' viewBox='0 0 280 75' preserveAspectRatio='none'>
                    <path
                      d={getAreaPathString(
                        getPoints(updatedMetrics.map((m) => m.latency), maxLatency).map((p) => ({ x: p.x / 2, y: p.y / 2 }))
                      )}
                      fill='#f59e0b'
                      fillOpacity='0.08'
                    />
